...
class GetDashboardDataViewModel(var repository: Repository) : ViewModel() {
    ...
    fun getTrending(limit: Int , skip :Int) {
        disposable.add(repository.getTrending(limit ,skip)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .doOnSubscribe { d -> responseLiveData.setValue(ApiResponse.loading()) }
            .subscribe(
    ...
    fun hitGetProviderData(token: String,id: String, service_id:String) {
        disposable.add(repository.getProviderData(token,id,service_id)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .doOnSubscribe { d -> responseLiveData.setValue(ApiResponse.loading()) }
            .subscribe(
